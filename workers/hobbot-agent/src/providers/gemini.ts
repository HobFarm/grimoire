// Gemini provider. Ported from Grimoire (workers/grimoire/src/gemini.ts).

import type { ModelContext, ModelEntry } from './types'

function buildGeminiUrl(ctx: ModelContext, model: string): string {
...
function buildDirectGeminiUrl(ctx: ModelContext, model: string): string {
  return `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${ctx.geminiKey}`
}

function buildGeminiHeaders(ctx: ModelContext): Record<string, string> {
...
function buildGeminiBody(prompt: string, options?: ModelEntry['options']) {
